...
lifetime = np.loadtxt("lifetime1.13")[:4500]

plt.plot(np.arange(len(lifetime)), lifetime, label="DRED")
plt.plot(np.arange(len(lifetime)), np.ones(len(lifetime))*1230, label='Greedy2')
plt.plot(np.arange(len(lifetime)), np.ones(len(lifetime))*1045, label='Greedy1')
plt.plot(np.arange(len(lifetime)), np.ones(len(lifetime))*639, label='Random')
...
# 显示图形
plt.savefig("lifetime1.13.png", bbox_inches='tight', pad_inches=0.05, dpi=600)